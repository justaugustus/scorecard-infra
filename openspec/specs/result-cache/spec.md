...
## Purpose
Defines the read-through orchestration seam between the HTTP surface and the scan
engine: serve a fresh stored result or produce one on demand, governed by a
freshness/TTL policy (commit-pinned immutable, latest by TTL), single-flight
de-duplication, a synchronous-with-timeout versus asynchronous response decision,
and provenance and determinism guarantees on every result.
## Requirements
