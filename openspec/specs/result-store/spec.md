# result-store Specification

## Purpose
Defines durable persistence of Scorecard results: a cloud-agnostic blob
abstraction (S3-compatible, Azure Blob, Google Cloud Storage, local filesystem,
in-memory) selected by a storage URL and configured entirely from the
environment, with a fixed object-key contract, canonical JSON2 bodies, and a
miss reported distinctly from a failure.
## Requirements
